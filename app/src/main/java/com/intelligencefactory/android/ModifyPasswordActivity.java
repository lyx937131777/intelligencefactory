...
//        Log.e("ModifyPassword","bt_re_pw:");
        ...
        bt_summit.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                final String new_pw = et_new_pw.getText().toString();
                String re_pw = et_re_pw.getText().toString();
                if (!new_pw.equals(re_pw))
                {
                    tv_message.setText("新密码不一致！");
                } else if(!pref.getString("password","").equals(old_pw))
                {
                    tv_message.setText("原密码错误！");

                }else
                {
                    tv_message.setText("");
                    String address = HttpUtil.LocalAddress+"/ModifyPassword";
                    HttpUtil.modifypasswordRequest(address, pref.getString("userID", "1"), new_pw, new Callback()
                    {
                        @Override
                        ...
                        public void onResponse(Call call, Response response) throws IOException
                        {
                            ...
                            if(responsDate.equals("true"))
                            {
                                ...
                                user.updateAll("userID = ?",pref.getString("userID",""));
                            }
                        }
                    });
    ...
    private void init_passwordFrame(final EditText et_pw, final Button bt_clear, final Button
            bt_eye)
    {
        // 监听文本框内容变化
        ...
        bt_eye.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                if (et_pw.getInputType() == InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD)
                {
                    ...
                    et_pw.setInputType(InputType.TYPE_CLASS_TEXT | InputType
